...
void send_string_f(char *string, FILE *output) /* includefile */
{
  int len;
  len=strlen(string);
...
void send_float_f(char *name,float floating_point, FILE *output) /* includefile */
{
  send_string_f(name, output);
  fwrite(&floating_point,sizeof(float),1,output);
  /*fprintf(stderr,"%f\n",floating_point);*/
}

void send_double_f(char *name, double double_precision, FILE *output) /* includefile */
{
  send_string_f(name, output);
...
void send_int_f(char *name, int integer, FILE *output) /* includefile */
{
  send_string_f(name, output);
...
void send_char_f(char *name, char integer, FILE *output) /* includefile */
{
  send_string_f(name, output);
...
void send_long_f(char *name, long integer, FILE *output) /* includefile */
{
  send_string_f(name, output);
...
void send_coords_f(double raj, double dej, double az, double za, FILE *output) /*includefile*/
{
  if ((raj != 0.0) || (raj != -1.0)) send_double_f("src_raj",raj, output);
  if ((dej != 0.0) || (dej != -1.0)) send_double_f("src_dej",dej, output);
  if ((az != 0.0)  || (az != -1.0))  send_double_f("az_start",az, output);
  if ((za != 0.0)  || (za != -1.0))  send_double_f("za_start",za, output);
}
...
while (i<argc) {
  ...
  /* broadcast the header parameters to the output stream */
    send_string_f("HEADER_START",output);
    send_string_f("rawdatafile",output);
    send_string_f(rawfilename,output);
    if (!strings_equal(source_name,"")) {
      send_string_f("source_name",output);
      send_string_f(source_name,output);
    }
    printf("sending machine_id..\n");
    send_int_f("machine_id",machine_id,output);
    send_int_f("telescope_id",telescope_id,output);
    send_coords_f(src_raj,src_dej,az_start,za_start,output);
      /* filterbank data */
      /* N.B. for dedisperse to work, foff<0 so flip if necessary */
    send_int_f("data_type",1,output);
    send_double_f("fch1",fch1,output);
    send_double_f("foff",foff,output);
    send_int_f("nchans",nchans,output);
    /* beam info */
    send_int_f("nbeams",nbeams,output);
    send_int_f("ibeam",ibeam,output);
    /* number of bits per sample */
    send_int_f("nbits",nbits,output);
    /* start time and sample interval */
    send_double_f("tstart",tstart,output);
    send_double_f("tsamp",tsamp,output);
    if (nifs == 1) {
      send_int_f("nifs",1, output);
    } else {
    ...
    send_string_f("HEADER_END",output);		
